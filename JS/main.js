  ...
  for (let r = 0; r < brick.row; r++) {
    for (let c = 0; c < brick.column; c++) {
      if (bricks[r][c].status) {
        ...
        } else if (arry.includes(bricks[r][c].index)) {
...
function barCollision() {
  ...
  let touchEdgeL =
    ball.x + ball.radius >= bar.x && ball.y + ball.radius > bar.y;
